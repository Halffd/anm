...
async function makeFurigana(text, mode = 'A') {
    ...
    const furigana = [];
    for (const token of tokens) {
        furigana.push(token.surface_form, token.reading) //token_to_dict(token, false, true, undefined, true));
    ...
    try {
        ...
        if (!dicts) {
            ...
            if (furi) {
                ...
                let fg;
                if (!kanji_matches) {
                    fg = token.surface_form;
                } else if (short) {
                    fg = [token.surface_form, hiragana_reading];
                } else {
                    fg = [token.surface_form, hiragana_reading, t ? t["reading"] : ""];
                }
                return fg;
            }
